...
import { CheckCircle2, Truck, Package, MessageCircle, Printer, ArrowRight } from 'lucide-react';
...
export default function OrderSuccessPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    fetchApi<Order>(`/orders/${params.id}`).then((res) => {
  ...
  if (!order) {
    return (
      <div className="max-w-xl mx-auto px-4 py-20 text-center space-y-4">
        ...
        <p className="text-xs text-zinc-500">Thank you! Your order #{params.id} has been recorded successfully.</p>
        <Link
          href="/shop"
